...
class CBF:
    ...
    def __init__(self,param):
        ...
        pass
        self.length_scale = 0.2    # found from  loop demo
        self.time = 0.0
    # Dynamics #
    ...
    def vis_barrier(
        ...
        for i, x_query in enumerate(vec_points):
            ...
            grad = self.dcbf_function(x_query, training_data, k_star_vec.T, K_inv, length_scale)
            U[i] = grad[0]
            V[i] = grad[1]
    ...
    def setObjects(self,distance,angle):
        ...
        # Every other
        x_lidar = cp.round(filtered_distance[::10] * cp.cos(filtered_angle[::10]).astype(cp.float32),5)
