...
class CBF:
    ...
    def __init__(self,param):
        ...
        pass
        self.length_scale = 0.095    # found from  loop demo
        self.time = 0.0
    # Dynamics #
    ...
    def cbf_function(self, K_star, k_inv):
        ...
        return  1 + (K_star.T @ k_inv @ (self.Y - 1))
    ...
    def constraints_cost(self,u_ref,x,y,theta,v):
        ...
        try:
            ...
            P_torch = torch.from_dlpack((H.toDlpack())).unsqueeze(0)   # Shape: (1, n, n)
            q_torch = torch.from_dlpack((f.toDlpack())).unsqueeze(0)   # Shape: (1, n)
            G_torch = torch.from_dlpack((A.toDlpack())).unsqueeze(0)   # Shape: (1, n_constraints, n)
            h_torch = torch.from_dlpack((b.toDLpack())).unsqueeze(0)   # Shape: (1, n_constraints)
