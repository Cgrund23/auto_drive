...
class ModelFreeCBF:
    ...
    def get_barrier_and_variance(self, p):
        ...
        Returns:
            h: Barrier value (scalar, clamped to reasonable range)
        ...
        # CRITICAL: Clamp barrier to prevent extreme values during startup
        # Barrier should be in [-2, 5] range to maintain QP feasibility
        h = max(-2.0, min(5.0, h))

        # GP posterior variance (Eq. 6 in paper)
        k_ss = self.rbf_kernel(p, p)[0, 0]
        sigma_sq = float(k_ss - k_star @ self._K_inv_cache @ k_star.T)
        sigma_sq = max(0.0, sigma_sq)  # Ensure non-negative
