...
class ControllerNode(Node):
    def __init__(self):
        ...
        # CRITICAL: Must be feasible even when q<0 (barrier violated)
        self.lambda_0 = 0.1  # Reduced to prevent infeasibility spiral
        self.lambda_1 = 0.1  # Reduced to prevent infeasibility spiral
        self.c_q = 0.01  # Minimal confidence to maximize feasibility
        ...
        # Goal for tangent controller (global frame)
        self.goal_x = 0.0  # Target x position
        self.goal_y = 5.0  # Target y position (5m forward in y)
        ...
        # Publisher for visualization data [q, qdot, B_q_v, B_q_omega]
        self.barrier_info_pub = self.create_publisher(Float32MultiArray, '/barrier_info', 10)

        self.get_logger().info('Model-Free CBF Node initialized')

    def tangent_controller(self):
        """
        Goal-seeking controller: navigates toward goal_x, goal_y in global frame.
        Uses LiDAR to detect obstacles and biases away from them.
        Returns: [v_ref, omega_ref]
        """
        # Compute error to goal in global frame
        dx = self.goal_x - self.x
        dy = self.goal_y - self.y
        distance_to_goal = float(cp.sqrt(dx**2 + dy**2))

        # If close to goal, slow down
        if distance_to_goal < 0.5:
            return [0.0, 0.0]  # Reached goal

        # Desired heading to goal (global frame)
        desired_theta = float(cp.arctan2(dy, dx))

        # Heading error (wrapped to [-pi, pi])
        heading_error = desired_theta - self.theta
        heading_error = float(cp.arctan2(cp.sin(heading_error), cp.cos(heading_error)))

        # Proportional control for heading
        K_heading = 2.0
        omega_ref = K_heading * heading_error

        # Velocity scales with heading error (slow down when turning)
        v_ref = 1.0 * (1.0 - 0.5 * abs(heading_error) / cp.pi)
        v_ref = max(0.3, v_ref)  # Minimum velocity

        # Obstacle avoidance bias using LiDAR
        if hasattr(self, 'last_ranges'):
            ...
            if len(front_ranges) > 0:
                ...
                close_obstacles = front_ranges < 1.0
                if cp.any(close_obstacles):
                    # Steer away from closest obstacle
                    close_angles = front_angles[close_obstacles]
                    close_ranges = front_ranges[close_obstacles]

                    # Weighted average of obstacle angles (closer = more weight)
                    weights = 1.0 / (close_ranges + 0.1)
                    avg_obstacle_angle = float(cp.sum(close_angles * weights) / cp.sum(weights))

                    # Add repulsive steering away from obstacle
                    K_avoid = 0.5
                    omega_ref -= K_avoid * avg_obstacle_angle

        # Clip omega to limits
    ...
    def lidar_callback(self, msg):
        ...
        if min_range < 0.35:
            ...
            # Safe - use reference (relaxed threshold from 0.5 to 0.3)
            u_safe = self.u_ref
        elif q_hat < -0.5:
            # Barrier significantly violated - likely GP initialization issue
            # Use reference but cap velocity for safety
            u_safe = [min(self.u_ref[0], 0.5), self.u_ref[1]]
            if not hasattr(self, '_violated_warning_count'):
                self._violated_warning_count = 0
            self._violated_warning_count += 1
            if self._violated_warning_count % 50 == 0:
                self.get_logger().warn(f'Barrier violated (q={q_hat:.2f}), using cautious reference')
        else:
        ...
        # Publish barrier info for visualization (low overhead)
        if self._log_counter % 10 == 0:  # Only every 10th iteration
            barrier_msg = Float32MultiArray()
            barrier_msg.data = [float(q_hat), float(qdot_hat),
                               float(B_q_hat[0]), float(B_q_hat[1])]
            self.barrier_info_pub.publish(barrier_msg)
...
def main(args=None):
    ...
    # Add visualization (runs at 5Hz in separate thread, won't slow control)
    from auto_drive.barrier_visualizer import add_visualization_to_controller
    visualizer = add_visualization_to_controller(node)
